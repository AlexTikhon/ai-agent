...
describe('createImageGenerationProvider', () => {
  it('defaults to MockImageGenerationProvider when IMAGE_GENERATION_PROVIDER_TOKEN is unset', () => {
  ...
  it('defaults to MockImageGenerationProvider when IMAGE_GENERATION_PROVIDER_TOKEN is empty', () => {
  ...
  it('throws a clear error for an unknown provider name', () => {
    expect(() =>
      createImageGenerationProvider({
        IMAGE_GENERATION_PROVIDER_TOKEN: 'stability',
      } as unknown as NodeJS.ProcessEnv),
    ).toThrow(/Unknown IMAGE_GENERATION_PROVIDER_TOKEN/);
  });
});
